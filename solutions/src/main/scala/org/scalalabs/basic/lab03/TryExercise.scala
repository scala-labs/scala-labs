...
object TryExercise01 {
  ...
  /** Implement the print method that should output the contents of the input
    * stream to STDOUT. Make sure to close the resource after reading it using
    * the close method provided by the {@code Closeable} interface. Handle
    * potential exceptions using Try and its recover functionality.
    *
    * Hint: You can use {@code Source.fromInputStream} to create a {@Code
    * BufferedSource} and use its convenience methods
    *
    ...
    *   - IOException during close: "Output: <contents of the stream>. Error:
    *     Failed to close! "
    */
