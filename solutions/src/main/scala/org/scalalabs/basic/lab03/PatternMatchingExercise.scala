...
/** This exercise introduces you to the powerful pattern matching features of
  * Scala.
  *
  * Pattern matching can in its essence be compared to Java's 'switch'
  * statement, even though it provides many more possibilites. Whereas the Java
  * switch statmenet lets you 'match' primitive types up to int's, Scala's
  * pattern matching goes much further. Practically everything from all types of
  * objects and Collections can be matched, not forgetting xml and a special
  * type of class called case classes.
  *
  ...
  * matching in combination with partial functions:
  * http://programming-scala.labs.oreilly.com/ch08.html#PartialFunctions
  */

object PatternMatchingExercise01 {

  /** ***********************************************************************
    * pattern matching exercises For expected solution see unittest
...
/** ***********************************************************************
  * Partial functions exercise. The MessageTransformer must use the
  * PartialFunction[Any, Any] called transform to transform messages in its
  * process(msg:Any) method
  *   - For every input message that can be transformed update the count using
  *     updateCount(...)
  *   - Messages that cannot be transformed must be returned as is, no count is
  *     updated. Provide an implementation only using PartialFunctions (if
  *     statements are not allowed) to make the unittest succeed. For expected
  *     behaviour see @PatternMatchingExerciseTest
  *     ***********************************************************************
  */
...
object PatternMatchingExerciseOther {
  ...
  /** ***********************************************************************
    * Pattern matching with partial functions For expected solution see
    * \@PatternMatchingExerciseTest
    * ***********************************************************************
    */
  ...
  val pf2: PartialFunction[String, String] = { case "other stuff" =>
    "Got stuff"
  }
