...
/** This excersice introduces you to Scala implicit conversion features.
  *
  * Scala has a nice feature that automatically lets you convert types and add
  * methods to an existing class. For instance, it is possible to write
  * "Hello".toList, which yields List(H, e, l, l, o) even though the
  * implementation of the String class does not provide a toList method. This is
  * coined 'library pimping' and is achieved via implicit conversions. In this
  * exercise, you will among other try out some implicit conversions from
  * integers to Joda's DateTime, so we can write little DSL like statements like
  * 1 day + 2 hours.
  *
  * Provide a suitable implementation in order to make the corresponding
  * unittest work.
  *
  * Reference material to solve these exercises can be found here: Implicit
  * conversions: http://programming-scala.labs.oreilly.com/ch08.html#Implicits
  */
...
object ImplicitConversionExercise01 {
  ...
  object Exercise02 {
    ...
    /** Use this conversion helper to convert fahrenheit to degree celsius and
  ...
  object Exercise03 {
    ...
    /** Use an implict conversion from string to an anonymous object that
      * contains a camelCase method. By doing so, the camelCase method is added
      * to the string
      */
