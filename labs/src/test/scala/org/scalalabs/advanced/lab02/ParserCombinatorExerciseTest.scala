...
/**
 * @see ParserCombinatorExerciseTest
 */
...
class ParserCombinatorExerciseTest extends Specification {
  ...
  "parser combinator exercise" should {
    ...
    "parse preposition phrase" in {
      ...
      parseAll(prepositionPhrase, "over the lazy quick dog").successful should beTrue

      parseAll(prepositionPhrase, "over dog").successful should beFalse
      parseAll(prepositionPhrase, "the quick brown fox").successful should beFalse
      parseAll(prepositionPhrase, "over brown dog").successful should beFalse
      parseAll(prepositionPhrase, "over the dog brown").successful should beFalse
    }
    ...
    "parse sentence" in {
      ...
      parseAll(sentence, "the quick dog jumps the lazy dog").successful should beTrue
      parseAll(sentence, "the quick brown fox jumps over the lazy dog").successful should beTrue

      parseAll(sentence, "the quick brown fox jumps over dog").successful should beFalse
      parseAll(sentence, "fox jumps over the lazy dog").successful should beFalse
      parseAll(sentence, "quick the brown fox jumps over the lazy dog").successful should beFalse
      parseAll(sentence, "jumps the quick brown fox over the lazy dog").successful should beFalse
      parseAll(sentence, "the quick brown fox jumps over the lazy").successful should beFalse
      parseAll(sentence, "the quick brown jumps fox over the lazy dog").successful should beFalse
    }
    ...
    "parse multiple additions" in {
      ...
      parseAll(math, "6.3 + 200 + 9.0 + 8 + 0.2257 + 15 + 0 + 56").get should be ~ (294.5257, 0.0001)
    }

    "parse simple arithmetic" in {
      parseAll(math, "-2 - 10 + 34 - 5").get ==== -41.0
      parseAll(math, "1.3 + 6.2 - 1.6 + 87.256").get ==== -81.356
      parseAll(math, "-6.3 + 200 + 9.0 - 8 + 0.2257 + 15 + 0 - 56").get should be ~ (235.4743, 0.0001)
